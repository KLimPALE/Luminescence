...
class CalibrationConfiguration:
    def __init__(self):
        ...
        self.end_wavelength_nanometers = 1314.1
...
class CalibrationManager:
    def __init__(self):
        ...
        print(f"[DEBUG] base_path: {self.base_path}")
    ...
    def connect_instruments(self, log_widget: tkinter.Text) -> bool:
        ...
        print("[DEBUG] Начало подключения оборудования")
        
        try:
            print("[DEBUG] Подключение монохроматора...")
            ...
            if self.chromator_device.connect():
                ...
                print("[DEBUG] Монохроматор подключен успешно")
            else:
                log_widget.insert(tkinter.END, "Монохроматор НЕ подключен\n")
                print("[DEBUG] Монохроматор НЕ подключен")
                connection_success = False
        except Exception as error:
            log_widget.insert(tkinter.END, f"Ошибка подключения монохроматора: {error}\n")
            print(f"[DEBUG] Ошибка монохроматора: {error}")
            connection_success = False
        
        try:
            print("[DEBUG] Подключение осциллографа...")
            
            import pyvisa
            print(f"[DEBUG] PyVISA version: {pyvisa.__version__}")
            
            try:
                rm_default = pyvisa.ResourceManager()
                resources_default = rm_default.list_resources()
                print(f"[DEBUG] Default VISA ресурсы: {resources_default}")
            except Exception as e:
                print(f"[DEBUG] Ошибка получения ресурсов через Default: {e}")
            
            try:
                rm_py = pyvisa.ResourceManager('@py')
                resources_py = rm_py.list_resources()
                print(f"[DEBUG] PyVISA-py ресурсы: {resources_py}")
            except Exception as e:
                print(f"[DEBUG] Ошибка получения ресурсов через @py: {e}")
            
            self.oscilloscope_device = Oscilloscope()
            
            print("[DEBUG] Попытка подключения осциллографа с таймаутом 30 сек...")
            if self.oscilloscope_device.connect(timeout_milliseconds=30000):
                ...
                print("[DEBUG] Осциллограф подключен успешно")
            else:
                log_widget.insert(tkinter.END, "Осциллограф НЕ подключен\n")
                print("[DEBUG] Осциллограф НЕ подключен")
                connection_success = False
        except Exception as error:
            log_widget.insert(tkinter.END, f"Ошибка подключения осциллографа: {error}\n")
            print(f"[DEBUG] Ошибка осциллографа: {error}")
            connection_success = False
        
        print(f"[DEBUG] Результат подключения: {connection_success}")
        return connection_success
    
    def disconnect_instruments(self, log_widget: tkinter.Text):
        print("[DEBUG] Отключение оборудования...")
        ...
        if self.chromator_device:
            try:
                ...
                print("[DEBUG] Монохроматор отключен")
            except Exception as error:
                log_widget.insert(tkinter.END, f"Ошибка отключения монохроматора: {error}\n")
                print(f"[DEBUG] Ошибка отключения монохроматора: {error}")
        ...
        if self.oscilloscope_device:
            try:
                ...
                print("[DEBUG] Осциллограф отключен")
            except Exception as error:
                log_widget.insert(tkinter.END, f"Ошибка отключения осциллографа: {error}\n")
                print(f"[DEBUG] Ошибка отключения осциллографа: {error}")
    
    def apply_device_settings(self, log_widget: tkinter.Text):
        print("[DEBUG] Применение настроек устройств...")
        ...
        if self.chromator_device:
            try:
                ...
                print(f"[DEBUG] Щели: вход={self.configuration.input_slit_micrometers}, выход={self.configuration.output_slit_micrometers}")
                ...
                log_widget.insert(tkinter.END, f"Усреднение кадров: {self.configuration.oscilloscope_average_count}\n")
                print(f"[DEBUG] Усреднение: {self.configuration.oscilloscope_average_count}")
            except Exception as error:
                log_widget.insert(tkinter.END, f"Ошибка настройки монохроматора: {error}\n")
                print(f"[DEBUG] Ошибка настройки монохроматора: {error}")
        ...
        if self.oscilloscope_device:
            try:
                ...
                log_widget.insert(tkinter.END, f"Осциллограф настроен (усреднение {self.configuration.oscilloscope_average_count})\n")
                print(f"[DEBUG] Осциллограф настроен")
            except Exception as error:
                log_widget.insert(tkinter.END, f"Ошибка настройки осциллографа: {error}\n")
                print(f"[DEBUG] Ошибка настройки осциллографа: {error}")
    
    def measure_signal_integral(self) -> float:
        if not self.oscilloscope_device:
            print("[DEBUG] Осциллограф не подключен, возврат 0")
            return 0.0
        
        print("[DEBUG] Захват сигнала с осциллографа...")
        ...
        if not voltage_values:
            print("[DEBUG] Нет данных с осциллографа")
            return 0.0
        
        print(f"[DEBUG] Получено {len(time_values)} точек сигнала")
        print(f"[DEBUG] Диапазон времени: {min(time_values):.2e} - {max(time_values):.2e} с")
        print(f"[DEBUG] Диапазон напряжения: {min(voltage_values):.3f} - {max(voltage_values):.3f} В")
        ...
        if baseline_signal:
            ...
            print(f"[DEBUG] Уровень фона: {baseline_level:.6f} В")
            for voltage in voltage_values:
                corrected_signal.append(voltage - baseline_level)
        else:
            print("[DEBUG] Фоновый интервал не найден, вычитание фона не выполнено")
        ...
        if len(integration_time) < 2:
            print("[DEBUG] Недостаточно точек для интегрирования")
            return 0.0
        
        integral_value = integrate_signal(integration_time, integration_signal)
        print(f"[DEBUG] Интеграл сигнала: {integral_value:.6e} В·с")
    ...
    def scan_spectrum(self, log_widget: tkinter.Text, progress_callback) -> List[Tuple[float, float]]:
        ...
        print(f"[DEBUG] Сканирование спектра: {len(wavelength_list)} точек")
        print(f"[DEBUG] Диапазон: {self.configuration.start_wavelength_nanometers} - {self.configuration.end_wavelength_nanometers} нм")
        print(f"[DEBUG] Шаг: {self.configuration.wavelength_step_nanometers} нм")
        
        for point_index, wavelength in enumerate(wavelength_list):
            print(f"[DEBUG] Установка длины волны {wavelength:.1f} нм...")
            ...
            print(f"[DEBUG] Точка {point_index+1}/{total_points}: {wavelength:.1f} нм -> {signal_integral:.6e} В·с")
        
        print(f"[DEBUG] Сканирование завершено")
    ...
    def find_spectral_peaks(self, spectrum_data: List[Tuple[float, float]]) -> List[Tuple[float, float]]:
        ...
        print(f"[DEBUG] Поиск пиков в спектре")
        print(f"[DEBUG] Максимальная интенсивность: {maximum_intensity:.6e}")
        print(f"[DEBUG] Порог: {intensity_threshold:.6e} ({self.configuration.signal_amplitude_threshold*100:.0f}%)")
        ...
        for index in range(1, len(intensity_array) - 1):
            if intensity_array[index] > intensity_array[index - 1] and intensity_array[index] > intensity_array[index + 1]:
                if intensity_array[index] > intensity_threshold:
                    ...
                    print(f"[DEBUG] Пик найден: {wavelength_array[index]:.2f} нм, {intensity_array[index]:.6e}")
        
        print(f"[DEBUG] Всего найдено пиков: {len(detected_peaks)}")
    ...
    def compute_calibration_coefficients(self, measured_peaks: List[Tuple[float, float]], 
        ...
        print(f"[DEBUG] Расчёт калибровочных коэффициентов")
        print(f"[DEBUG] Измеренные пики: {len(measured_peaks)}")
        print(f"[DEBUG] Эталонные пики: {len(reference_peaks)}")
        ...
        for reference_wavelength, reference_name in reference_peaks:
            ...
            if abs(nearest_peak[0] - reference_wavelength) < self.configuration.peak_matching_tolerance_nanometers:
                ...
                print(f"[DEBUG] Сопоставление: эталон {reference_wavelength:.1f} -> измерено {nearest_peak[0]:.2f} (ошибка {nearest_peak[0] - reference_wavelength:.3f})")
        ...
        if len(matched_points) == 1:
            ...
            print(f"[DEBUG] Одноточечная калибровка: сдвиг {wavelength_offset:.3f} нм")
        else:
            ...
            print(f"[DEBUG] Линейная калибровка: slope={slope_factor:.6f}, intercept={intercept_nanometers:.3f}")
        ...
        print(f"[DEBUG] Статистика: средняя ошибка={calibration_result['statistics']['mean_error_nanometers']:.3f} нм")
    ...
    def apply_calibration_to_spectrum(self, spectrum_data: List[Tuple[float, float]], calibration_result: dict) -> List[Tuple[float, float]]:
        ...
        print(f"[DEBUG] Применена калибровка к {len(calibrated_spectrum)} точкам спектра")
    ...
    def save_calibration_data(self, calibration_data: dict) -> Path:
        ...
        print(f"[DEBUG] Калибровка сохранена: {calibration_filepath}")
        ...
        print(f"[DEBUG] Файл конфигурации: {config_filepath}")
    ...
    def load_custom_reference_file(self, file_path: Path) -> List[Tuple[float, str]]:
        ...
        print(f"[DEBUG] Загружено {len(loaded_peaks)} эталонных пиков из {file_path}")
        return loaded_peaks
    ...
    def save_parameters_to_file(self, file_path: Path):
        ...
        print(f"[DEBUG] Параметры сохранены: {file_path}")
    ...
    def load_parameters_from_file(self, file_path: Path):
        ...
        print(f"[DEBUG] Параметры загружены: {file_path}")
...
class CalibrationApplication:
    ...
    def initialize_user_interface(self):
        print("[DEBUG] Запуск GUI приложения калибровки")
    ...
    def toggle_calibration_mode(self):
        ...
        if self.is_automatic_mode:
            ...
            print("[DEBUG] Переключено в автоматический режим")
        else:
            ...
            print("[DEBUG] Переключено в ручной режим")
    ...
    def load_parameters_from_file(self):
        ...
        if file_path:
            try:
                ...
                print(f"[DEBUG] Ошибка загрузки параметров: {error}")
    ...
    def update_parameters_from_entries(self):
        try:
            ...
            print("[DEBUG] Параметры обновлены из полей ввода")
    ...
    def connect_instruments(self):
        def connection_task():
            print("[DEBUG] Запуск задачи подключения")
            ...
            if connection_successful:
                ...
                print("[DEBUG] Оборудование готово к калибровке")
            else:
                self.append_to_log("\nПроверьте подключение приборов\n")
                print("[DEBUG] Оборудование НЕ готово")
    ...
    def disconnect_instruments(self):
        def disconnection_task():
            print("[DEBUG] Запуск задачи отключения")
            ...
            self.disconnect_button.config(state=tkinter.DISABLED)
            self.start_calibration_button.config(state=tkinter.DISABLED)
            print("[DEBUG] Оборудование отключено")
    ...
    def draw_spectrum(self):
        ...
        def wavelength_to_x(wavelength):
            ...
            return margin_left + (wavelength - min_wavelength) / (max_wavelength - min_wavelength) * plot_width
        
        def intensity_to_y(intensity):
            return margin_top + plot_height - (intensity / max_intensity) * plot_height
        ...
        for x in range(0, 6):
            wavelength = min_wavelength + (max_wavelength - min_wavelength) * x / 5
        ...
        for y in range(0, 5):
            ...
            self.graph_canvas.create_text(margin_left - 5, y_position, text=f"{intensity:.2e}" if intensity < 1 else f"{intensity:.2f}", anchor="e", font=("Arial", 8))
    ...
    def start_calibration_process(self):
        def calibration_task():
            print("[DEBUG] Запуск процесса калибровки")
            ...
            try:
                if not self.is_automatic_mode:
                    ...
                    print("[DEBUG] Ручной режим: параметры обновлены")
                ...
                if self.is_automatic_mode:
                    for reference_data in self.calibration_manager.builtin_reference_data.values():
                        ...
                        print(f"[DEBUG] Используются встроенные данные: {reference_data['description']}")
                        break
                else:
                    ...
                    try:
                        ...
                        print(f"[DEBUG] Загружено {len(reference_peaks)} эталонных пиков")
                    except Exception as error:
                        self.append_to_log(f"Ошибка загрузки эталонных данных: {error}\n")
                        print(f"[DEBUG] Ошибка загрузки эталонных данных: {error}")
                        self.start_calibration_button.config(state=tkinter.NORMAL)
                        self.connect_button.config(state=tkinter.NORMAL)
                        self.disconnect_button.config(state=tkinter.NORMAL)
                        self.mode_button.config(state=tkinter.NORMAL)
                ...
                print(f"[DEBUG] Диапазон сканирования: {self.calibration_manager.configuration.start_wavelength_nanometers} - {self.calibration_manager.configuration.end_wavelength_nanometers} нм")
                ...
                if "error_message" in self.calibration_result:
                    ...
                    print(f"[DEBUG] Ошибка расчёта: {self.calibration_result['error_message']}")
                else:
                    ...
                    print(f"[DEBUG] Калибровка завершена успешно")
            
            except Exception as error:
                self.append_to_log(f"\nОшибка: {error}\n")
                print(f"[DEBUG] Ошибка в процессе калибровки: {error}")
                import traceback
                traceback.print_exc()
...
if __name__ == "__main__":
    print("[DEBUG] Запуск приложения калибровки")
